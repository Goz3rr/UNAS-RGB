...
int main(int argc, char **argv) {
	...
	handle = usbOpenDevice(0x16C0, "u-nas.com", 0x05DC, "RGB-LED");
	if(handle == NULL) {
	...
	if(strcmp(argv[1], "off") == 0) {
		nBytes = usb_control_msg(handle,
			...
			USB_LED_SETLED, 0, 0, (char *)buffer, sizeof(buffer), 5000);
		...
		for (int decColour = 0; decColour < 3; decColour += 1) {
			...
			for(int i = 0; i < 255; i += 1) {
				...
				nBytes = usb_control_msg(handle,
					USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN,
					USB_LED_SETLED, (uint16_t)rgbColour[0] | ((uint16_t)rgbColour[1] << 8), rgbColour[2], (char *)buffer, sizeof(buffer), 5000);
				usleep(5000);
			}
		}
	} else {
		...
		nBytes = usb_control_msg(handle,
            USB_TYPE_VENDOR | USB_RECIP_DEVICE | USB_ENDPOINT_IN,
			USB_LED_SETLED, (uint16_t)r | ((uint16_t)g << 8), b, (char *)buffer, sizeof(buffer), 5000);
	}
